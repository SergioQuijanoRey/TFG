...
def euclidean_distance_matrix(x: torch.Tensor) -> torch.Tensor:
    ...
    NOTE: copied from https://tinyurl.com/2daxxeor
    """
    # step 1 - compute the dot product

    # shape: (batch_size, batch_size)
    x_t = x.t()
    dot_product = torch.mm(x, x_t)
    ...
    # shape: (batch_size, batch_size)
    distance_matrix = (
        squared_norm.unsqueeze(0) - 2 * dot_product + squared_norm.unsqueeze(1)
    )
...
# NOTE -- copied from https://github.com/lyakaap/NetVLAD-pytorch/blob/master/hard_triplet_loss.py
# NOTE -- implements the first blog of #72 but using pytorch instead of tensorflow
