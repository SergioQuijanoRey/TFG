...
En esta sección vamos a introducir algunas de las herramientas matemáticas sobre las que se apoya nuestro trabajo. Estas herramientas son básicas y no aportan nada nuevo al estudio que hacemos sobre las redes neuronales, por lo que el lector puede pasar directamente a \customref{ch:modelizacion}

\section{Tensores}

\subsection{Notación}

% TODO -- en la pagina 23 se añade una notacion que parece ser muy comoda
% TODO -- Tau_{N, M} el espacio euclídeo de los tensores con entradas reales, orden N y dimensión M en todos los modos
...
Al espacio de tensores de orden $N$ y dimensión $M$ en cada modo (de nuevo, \customref{sec:deftensor}) lo denotaremos por $\espaciotensores{N}{M}$. Al espacio de matrices de dimensiones $p, q$ lo denotaremos, como es usual, como $\espaciomatrices{p}{q}$.
...
\todo{Realmente no sé si esto es claro, por culpa de estar usando $*$, pero seguramente sea directo comprobarlo}

Veamos ahora algunos ejemplos para familiarizarnos con este espacio.

\begin{ejemplo} \label{ej:prod_formal}
\todo{No sé si estos ejemplos sobran del documento al ser muy simples. Los he introducido porque los usé al principio para entender mejor los tensores, pero ahora no sé si tiene sentido dejar esto aquí}
...
\begin{proposicion}[Producto tensorial de dos vectores]
    ...
    $$\mathbf{v} \otimes \mathbf{w} = \mathbf{v} \mathbf{w}^T \in \espaciomatrices{N}{M}$$
...
Sean $\mathbf{v} = (v_1, \ldots, v_N)$ y $\mathbf{w} = (w_1, \ldots, w_M)$, ambos no nulos. Entonces, usando la expresión del producto tensorial que introducimos en \customref{sec:otra_forma_tensores}, tenemos que:
