...
\section{Conceptos básicos de Análisis} \label{sec:preliminares_funcional}
\todo{JMeri: el nombre original que tenía esta sección era 'Análisis Funcional'. Aunque ya lo haya cambiado, revisarlo porque creo que el nombre de la sección es bastante relevante}
...
A continuación, introduciremos algunos conceptos sobre espacios de funciones que serán fundamentales en la \sectionref{sec:justificacion_func_repr}. Empezamos introduciendo el siguiente espacio de funciones, que es de sobra conocido:
...
\begin{observacion}
    ...
    \todo{Escribir el teorema de proyección ortogonal porque ya es la segunda vez que lo menciono.}
...
\begin{proof}
    Para realizar la prueba, comprobamos que la operación definida es en efecto un producto escalar. Una vez hecho esto, aplicamos la completación del espacio en base al producto escalar y consideramos el resultado de este proceso como el espacio de Hilbert producto tensorial, llegando a lo que se busca.
    ...
    por ser producto de dos reales mayores o iguales que cero, y es cero sí y solo sí $\innerproduct{\phi_1}{\phi_1}_1 = 0$ ó $\innerproduct{\phi_2}{\phi_2}_2 = 0$, en cuyo caso, $\phi_1 = 0$ ó $\phi_2 = 0$. Y usando la \propref{prop:cero_tensorial_vector} sabemos que entonces $\phi_1 \otimes \phi_2 = 0$.
...
\begin{proof}
    \todo{JMeri: importante demostrar esta proposición porque la uso más tarde.}
\end{proof}
...
\end{figure}
\todo{Mejorar la calidad de esta imagen}
...
\todo{Antes tenía las \textit{wavelets} pero las he quitado porque no aportaban nada. Asegurarme de que no las estoy mencionando}
