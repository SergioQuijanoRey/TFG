...
\begin{itemize}
    ...
    \item El superíndice $\gamma$ indica con qué tensor de la capa $l$ y posición $j$ estamos trabajando. Es decir, podemos tener más de un tensor en un nivel y posición dentro del nivel
...
El siguiente diagrama muestra el proceso de generación de los tensores. Por simplicidad, suponemos que $r = 1$. Es decir, que en cada localización $j$ solo tenemos un tensor, independientemente del nivel $l$ en el que nos encontremos.
...
\begin{tikzpicture}[
    ...
    \node (capa_0_pos_1){$\varphi^{j = 1, \gamma = 1}$};
    \node (capa_0_pos_2)[right=of capa_0_pos_1]{$\varphi^{j = 2, \gamma = 1}$};
    \node (capa_0_pos_3)[right=of capa_0_pos_2]{$\varphi^{j = 3, \gamma = 1}$};
    \node (capa_0_pos_4)[right=of capa_0_pos_3]{$\varphi^{j = 4, \gamma = 1}$};

    \node (capa_1_pos_1)[below=of capa_0_pos_2]{$\phi^{l = 1, j = 1, \gamma = 1}$};
    \node (capa_1_pos_2)[right=of capa_1_pos_1]{$\phi^{l = 1, j = 2, \gamma = 1}$};

    \node (capa_2_pos_1)[below=of capa_1_pos_1]{$\mathcal{A}^y = \phi^{l = 2, j = 1, \gamma = 1}$};

    \draw[-stealth] (capa_0_pos_1.south) -- (capa_1_pos_1.north);
    \draw[-stealth] (capa_0_pos_2.south) -- (capa_1_pos_1.north);

    \draw[-stealth] (capa_0_pos_3.south) -- (capa_1_pos_2.north);
    \draw[-stealth] (capa_0_pos_4.south) -- (capa_1_pos_2.north);

    \draw[-stealth] (capa_1_pos_1.south) -- (capa_2_pos_1.north);
    \draw[-stealth] (capa_1_pos_2.south) -- (capa_2_pos_1.north);

\end{tikzpicture}
    \caption{Ejemplo gráfico del proceso de construcción del tensor $\mathcal{A}^y$ a través de la descomposición \textit{HT}. Por simplicidad, estamos suponiendo que $r = 1$}
\end{figure}
\todo{Explicar más cosas en la caption. Por ejemplo, valores de los parametros como capas, orden de los tensores, ...}
