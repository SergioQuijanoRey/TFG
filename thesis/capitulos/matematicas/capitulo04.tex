...
\todo{Ya he hablado de esto, pero por darle la misma estructura que en HT}
...
Al igual que ocurría en el modelo \textit{CP}, nuestras convoluciones $1 \times 1$ son dependientes de la localización. En la práctica, las convoluciones en los modelos convolucionales son independientes de la localización, lo que se conoce como \textbf{compartición de coeficientes}.

La compartición de coeficientes es necesaria porque identificar ciertos patrones en la imagen no debería depender de la localización del patrón en la imagen. Por ejemplo, una cara debería detectarse como tal independientemente de la localización de esta.
...
Como ocurría en \customref{subs:comparticion_parametros_cp}, la compartición de parámetros provoca que nuestro modelo pierda la universalidad. Aunque en este caso, los tensores que podemos generar con la compartición de parámetros no están limitados a ser simétricos, como en el caso \textit{CP}. Esto ya es un primer indicativo de que el modelo \textit{HT} es más expresivo que el modelo \textit{CP}.
