...
Recordemos que en \customref{def:tensor_puro} hemos definido estos tensores como \textbf{tensores puros}. La anterior proposición muestra que los tensores puros que vienen dados como producto tensorial de dos vectores tienen un rango matricial igual a uno (en caso de que no sean ambos vectores nulos).
...
\begin{proof} \label{proof:demostracion_cp_es_universal}
    ...
    \begin{equation}
        \mathcal{A} = \sum_{d_1 = 1}^{M_1} \sum_{d_2 = 1}^{M_2} \cdots \sum_{d_N = 1}^{M_N} \lambda_{i_1, \ldots, i_N} \cdot \vec{\delta}_{i_1, M_1} \otimes \ldots \otimes \vec{\delta}_{i_N, M_N}
    \end{equation}
\end{proof}
...
\begin{observacion}[Descomposición conjunta] \label{observacion:descomposicion_cp_conjunta}
    ...
    Notar que en la demostración hemos fijado un cojunto de vectores independientemente del tensor que queremos descomponer. Es decir, hemos fijado el conjunto de vectores

    % TODO -- seguir aqui

    \begin{equation}
        \conjunto{\delta_{j, N}: \; N \in \N}
    \end{equation}
