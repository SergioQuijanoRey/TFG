...
\begin{proof}
    ...
    \todo{No he visto el caso en la que $A(x) = B(x)$}
\end{proof}
...
\begin{proof}
    ...
    Con esto, basta probar que la medida de $S^{\nv{y}}$ es nula casi por doquier, porque en tal caso $S \subseteq S^{\nv{y}} \times \R^d$ siendo $S^{\nv{y}} \times \R^d$ de medida nula y podemos aplicar el segundo apartado de \customref{lema:prop_fundamentales_medida}. Para ello definimos también
    ...
    La idea de este conjunto es
    \todo{Definir la idea de este conjunto. En el papel de JMeri daba una explicación muy buena de este}
...
\begin{teorema}[Teorema principal] \label{teorema:teorema_principal_especificacion}
\end{teorema}
\begin{proof}
...
\begin{lema}[TODO -- dar nombre] \label{lema:lema_previo_corolario}
...
\begin{proof}
    En \customref{teorema:teorema_principal_especificacion} hemos visto que un tensor generado por un modelo \textit{HT} tiene rango \textit{CP} de al menos $\min \conjunto{r_0, M}^{\frac{N}{2}}$  en casi todo punto del espacio de parámetros que definen el modelo \textit{HT}, $\conjunto{\nv{a^{l, j, \gamma}}}_{l, j, \gamma}$. Es decir, que el modelo \textit{CP} requerirá un tamaño exponencial: $Z \geq \min \conjunto{r_0, M}^{\frac{N}{2}}$.
    ...
    Usando este hecho, aplicamos el \customref{lema:lema_previo_corolario}:
    ...
    El teorema \customref{teorema:teorema_principal_especificacion} nos dice que con probabilidad 1 que $\mathcal{A}^*$ no puede ser generado por un modelo \textit{CP} con menos de $\min \conjunto{r_0, M}^{\frac{N}{2}}$ canales en la capa oculta. Luego $\mathcal{A}^* \notin \conjunto{\mathcal{A}^\lambda: \lambda \in \Lambda}$.
    ...
    \todo{Hay que ver que el tamaño de un modelo \textit{CP} coincide con la desigualdad apropiada. Puede ser directo por la definición de \textit{CP rank}}
\end{proof}
