...
En esta sección introduciremos algunos resultados que serán usados en las demostraciones del teorema \propref{teorema:teorema_principal_especificacion} y el \propref{corolario:corolario_principal_concreto}. Empezamos con los siguientes resultados de teoría de la medida de Lebesgue:
...
\begin{observacion} A partir de la primera propiedad es directo ver que el producto de Kronecker verifica las mismas propiedades que el producto tensorial (véase \propref{prop:tensores_propiedades}). Es decir, sean $A, B, C$ tres matrices cualesquiera y $\lambda \in \R$. Entonces:
...
\begin{proof}
    ...
    Para ver esto tenemos en cuenta que $det(U_r(\nv{x}))$ es un polinomio con entradas en $x \in \R^{2MN + N}$. Por el \propref{lema:prop_fundamentales_medida} sabemos que entonces o bien es el polinomio idénticamente nulo o bien el conjunto de sus ceros tiene medida nula. Basta ver por tanto que no es el polinomio idénticamente nulo.
...
\begin{proof}
    ...
    Por la \propref{prop:rango_matricial_descomp_cp}, bastará probar que los rangos de las matrizaciones $[\phi^{l, j, \gamma}]$ tienen al menos rango matricial $r^{2^{l/2}}$ en casi todo punto (respecto al conjunto de los parámetros que definen el modelo \textit{HT}: $\conjunto{ \nv{a^{l, j, \gamma}}}_{l, j, \gamma}$). Comenzamos la inducción viendo el \textbf{caso $l = 1$}. Por definición, tenemos que para unos valores concretos de $j, \gamma$:
...
\begin{proof}
    ...
    y por tanto
    \todo{Obvio pero habrá que demostrarlo. No conocemos $\Lambda$ y podríamos tener una sucesión de distancias que converge a cero}
    ...
    Usando la \propref{prop:matrizacion_no_degenera_distancias} sabemos entonces que
...
\begin{proposicion} \label{prop:matrizacion_no_degenera_distancias}
    La matrización no degenera distancias euclídeas. Es decir, si dos tensores $\mathcal{A}$, $\mathcal{B}$ están a una distancia euclídea estrictamente positiva, las matrices $[\mathcal{A}]$, $[\mathcal{B}]$ no pueden estar a una distancia euclídea nula. O lo que es lo mismo,

    \begin{equation}
        d(\mathcal{A}, \mathcal{B}) = 0 \iif d([\mathcal{A}], [\mathcal{B}])
    \end{equation}
\end{proposicion}
