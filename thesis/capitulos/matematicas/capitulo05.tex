...
\begin{lema}[Algunas propiedades de la medida] \label{lema:prop_fundamentales_medida} Las siguientes propiedades sobre la medida de Lebesgue son ciertas:
...
\begin{proof}
    ...
    % TODO {Hay que probar o desarrollar mejor esta matrización}
    ...
    también tiene medida cero. Por tanto, $\phi_1 \times \R$ claramente tiene medida cero en $\R^{2M + 1}$. Esta dimensionalidad viene dada porque sabemos que $\nv{\varphi^{j, \alpha}} \in \R^M$. Luego $\phi^{1, j, \gamma} \in \R^{2M}$ y, con ello, $\phi_1^\gamma \in \R^{2M}$. Así pues, $\phi_1 \in \R^{2M}$ y claramente $\phi_1 \times \R \subseteq \R^{2M + 1}$. Finalmente deducimos que:
...
\begin{proof}
    ...
    Sabemos, por la \propref{prop:hilbert_convnorma_convcoef}, que convergencia en norma implica convergencia en los coeficientes de representación. Como hemos supuesto la convergencia en norma, con lo anterior tenemos que:
    ...
    y, al tomar límite, el rango puede disminuir pero no aumentar. Además, usando la \propref{prop:matrizacion_no_degenera_distancias} sabemos que la matrización no degenera distancias, con lo que concluimos que:
