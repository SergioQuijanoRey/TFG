...
\begin{proof}
    ...
    también tiene medida. Por tanto, $\phi_1 \times \R$ claramente tiene medida cero en $\R^{2M + 1}$. Esta dimensionalidad viene dada porque sabemos que $\nv{\varphi^{j, \alpha}} \in \R^M$. Luego $\phi^{1, j, \gamma} \in \R^{2M}$ y, con ello, $\phi_1^\gamma \in \R^{2M}$. Así pues, $\phi_1 \in \R^{2M}$ y claramente $\phi_1 \times \R \in \R^{2M + 1}$. Finalmente deducimos que:

    \begin{equation}
        \phi := \conjunto{\phi^{i, j, \gamma}: \; (j, \gamma) \in \deltaset{N/2} \times \deltaset{r_1}, \; rank([\phi^{1, j, \gamma}]) \neq r} \subseteq \phi_1 \times \R
    \end{equation}
    \todo{JMeri: esto son índices, es obvio que son finitos}
