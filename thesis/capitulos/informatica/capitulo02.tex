...
Las \textbf{redes neuronales} son modelos que usan unidades más pequeñas llamadas \textbf{neuronas}. Con el paso de los años han aparecido distintos tipos de redes neuronales.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.\textwidth]{informatica/neurona_mlp}
...
La capa convolucional es la herramienta que caracteriza este tipo de redes. Esta capa aplica filtros o \textit{kernels} a los datos con los que trabaja. A partir de estos filtros extraemos información de los datos de entrada.

Estos filtros ya se usaban antes de que el \textit{deep learning} se hiciese popular. Sin embargo, dichos filtros eran diseñados manualmente por un experto. En el enfoque del aprendizaje automático, estos filtros se aprenden a partir de los datos de entrenamiento.
...
Para aplicar una red \textit{MLP} a imágenes, podemos vectorizar la imagen de entrada (por ejemplo, tomando las columnas de la imagen y concatenando dichas columnas en un único vector). Esto es claramente ineficiente, porque en cada capa combinamos toda la información de toda la imagen. Para extraer características de la imagen no es útil tomar vecindarios no muy distantes entre sí, por lo que hemos comentado. Y aquí es donde las capas convolucionales son más eficientes.
...
Y a la hora de trabajar con imágenes, que podemos representar como matrices, podemos realizar una convolución entre funciones de dos variables:

\begin{equation}
    (f * g)(x, y) := \sum_{s \in \N} \sum_{t \in \N} f(s, t) g(x - s, y - t)
\end{equation}

Con todo esto ya tenemos las bases matemáticas para implementar una operación que aplique esta forma de aplicar filtros a imágenes. Una \textbf{capa convolucional} vendrá dada por los siguientes hiperparámetros:
...
Podemos mencionar algunas características muy interesantes de esta operación:
...
Este tipo de capa consiste en simplemente en aplicar ciertas capas de tipo \textit{MLP}. Como ya hemos visto en \sectionref{isubs:deep_learning_teoria}, ya hemos visto que necesitamos trabajar con datos en formato vectorial. Una posible solución es transformar los datos a un vector, operación que se conoce comúnmente como \textit{flatten}.

Estas capas se suelen poner al final de la red, para realizar la predicción final. Toman las características jerárquicas que las convoluciones extraen y las combinan para resolver la tarea planteada.
...
Es claro que nuestra solución forma parte del área de visión por computador. Buscamos comprender información en forma de imágenes (imágenes de rostros) para resolver una tarea (identificar personas independientemente de los cambios asociados al paso del tiempo). Y es aún más claro que esta solución está en la intersección de la visión por computador y el aprendizaje automático, pues usamos técnicas de \textit{machine learning} para construir un modelo que comprenda la información de las imágenes para resolver la tarea.

\section{Codificación de la identidad}

Buscamos que la sdljasdlñajsdjlasdjl

\subsection{\textit{Embedding} semántico}

\subsection{\textit{Triplet Loss} como función de pérdida para aprender el \textit{embedding}}

\section{Mejoras técnicas objeto de estudio}

\subsection{Enfoque online de \textit{Triplet Loss}}

\subsection{Generación online de \textit{batches} usando \textit{P-K sampling}}

\subsection{Selección de triples y variaciones en la función de pérdida}
