...
En \customref{isubs:normas_embeddings} y \customref{isec:margenes_suaves} hemos comentado el problema que puede suponer que la red aprenda a colapsar cualquier entrada al vector $\vec{0}$. Además de introducir un margen en las funciones de pérdida, otra técnica a explorar es la de normalizar las salidas de la red, forzando a que tengan norma unitaria. Esta técnica se usa en \cite{informatica:facenet}. Esta normalización consiste en realizar la siguiente sustitución:

\begin{equation}
    \hat{x} := \frac{f_{\theta}(x)}{||f_{\theta}(x)||_2}
\end{equation}

Además del interés que pueda tener estudiar el comportamiento de los modelos al introducir la normalización, esta técnica introduce el siguiente punto a tener en cuenta. Durante los entrenamientos hemos tenido muchos problemas para producir entrenamientos estables. Esto es, en la mayoría de ocasiones, el entrenamiento se saturaba tras pocas iteraciones. Según nuestra experiencia, introducir normalización ayuda enormemente a reducir este problema.
...
Esta clase toma una red neuronal como parámetro. El método \lstinline{forward}, que las redes neuronales de \lstinline{pytorch} debe implementar, usa dicho método en la clase base almacenando la salida. Sobre dicha salida, se realiza y devuelve la normalización.

El uso de este patrón permite que podamos decidir si queremos trabajar o no con normalización, sin que el resto del código tenga que saber esto. Por ejemplo:
