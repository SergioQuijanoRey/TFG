...
\todo{Esto está en una sección posterior, así que de momento no hemos hablado de él}
...
\lstinline{FastRetrievalAdapter} claramente está utilizando el patrón \textit{composite}, del que ya hemos hablado en \customref{isec:loggin_metricas}. Para acelerar los tiempos de ejecución, simplemente realizamos un pre-cómputo y empleamos la implementación de clases previas.
\todo{Estamos usando el patrón Decorator y no composite. No es el patrón adapter porque no estamos cambiando el comportamiento para acoplarnos a otra interfaz}
...
Hemos iterado sobre distintas bases de datos, aumentando la complejidad y tamaño de estas, sin preocuparnos demasiado sobre la eficiencia de la base de código, hasta que hemos encontrado problemas considerables debidos a los tiempos de ejecución. Una vez que nos encontramos con tiempos de ejecución demasiado lentos, identificamos qué partes del código son las responsables de estos tiempos. Y con ello, sabemos en qué partes concretas debemos concentrar nuestros esfuerzos. De esta forma desarrollamos una amplia base de código inicial de forma rápida, sin aplicar optimizaciones prematuras. Y las optimizaciones que realizamos son óptimas, pues estamos localizando el código responsable de los tiempos de cómputo.

Todo esto se fundamenta en la ley de Amdahl \cite{informatica:amdahl_original} \cite{informatica:amdahl_moderno}. Si optimizamos una fracción de cómputo $f$ obteniendo una mejora en tiempos de cómputo de $S$ para esa fracción, la mejora global viene dada por:

\begin{equation} \label{eq:amdahl}
    S_{global} := \frac{1}{(1 - f) + \frac{f}{S}}
\end{equation}

Además, suponiendo que optimicemos la fracción de cómputo hasta tal punto que sea instantánea de computar, la mejora global sería:

\begin{equation}
    S_{máxima} := \lim_{S \to \infty}\frac{1}{(1 - f) + \frac{f}{S}} = \frac{1}{1 - f}
\end{equation}

Esto deja de manifiesto que no debemos fijarnos únicamente en lo buena que sea una optimización, sino que también debemos encontrar aquellas partes del código que mayor tiempo de cómputo (mayor valor de $f$) estén suponiendo. Esto motiva al uso de \textit{flamegraphs} para localizar las funciones a optimizar.

Identificamos problemas en los tiempos de cómputo trabajando con el \textit{dataset} \textit{LFW}. El proceso de identificación y resolución se agrupan en las issues \cite{informatica:issue35_profiling} \cite{informatica:issue36_slowmetrics} y resuelve en la pull request \cite{informatica:pr42_profiling}.

El commit 6aade342e0736d911040d969f9e69bebc49d41ef indica el estado original previo a realizar las optimizaciones. Usando el paquete \lstinline{cProfile}, de la librería estándar de \lstinline{Python}, añadimos la opción de realizar un \textit{profile} del proceso de entrenamiento. Realizamos un primer \textit{profile}. Guardamos los resultados en el archivo \lstinline{./src/profiling/first_profile.txt} los resultados. Filtramos estos resultados en el archivo \lstinline{./src/profiling/first_profile_filtered.txt}, para que contenga únicamente entradas asociadas a nuestra librería, que es el código que podemos modificar. De esta forma, podemos leer este archivo para buscar algunas conclusiones. Pero es mucho más útil si utilizamos gráficos interactivos tipo \textit{flamegraph}

Podemos explorar, a través de una terminal interactiva, los archivos guardados con el comando \lstinline{python -m pstats <file_path>}. Pero es mucho más cómodo usar \textit{flamegraphs} interactivos. Para ello, usamos el comando \lstinline{snakeviz <file_path>}.
\todo{No está bien. Tenemos archivos \lstinline{first_profile.stat}}

\subsection{Primer \textit{profile}}

En este \textit{profile} usamos \textit{lazy data augmentation} (véase \customref{isec:aumentado_datos}) con los valores $P = 200, K = 2$. A partir de ello, obtenemos los siguientes resultados, que visualizamos con \lstinline{snakeviz}:

\begin{figure}[H]
\centering
    \begin{subfigure}{.4\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{informatica/profiles/first_profile/global}
        \caption{Visualización global de todo el proceso de entrenamiento}
        \label{img:first_profile_global}
    \end{subfigure}%
    \begin{subfigure}{.6\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{informatica/profiles/first_profile/dataloader}
...
Como era de esperar por lo comentado en \customref{isec:aumentado_datos}, el enfoque \textit{lazy} conlleva tiempos de ejecución mayores. Solo estamos midiendo el tiempo de ejecución en el entrenamiento. Y en este, el enfoque clásico ya tiene todas las imágenes pre-computadas, mientras que el enfoque \textit{lazy} las tiene que generar a demanda. Sin embargo, acabamos de ver que permite usar unos valores $\{P, K\}$ más altos. Y además, la diferencia no es demasiado grande: menos de un minuto de diferencia en tiempos totales de algo más de 15 minutos.
...
\end{table}
\todo{La tabla se sale por la derecha}
...
En \customref{table:optimization_process_second} podemos ver que no estamos mostrando tiempos de cómputo. Esto es porque el uso de la función de \lstinline{pytorch} para computar todos los \textit{embeddings} de golpe provoca que el consumo de memoria se eleve hasta abortar el proceso. Además, los \textit{benchmarks} han empeorado algo.

Sin embargo, mantenemos las dos implementaciones, permitiendo elegir cuál usar en cada momento. Puede ser que al acceder a los servidores \textit{nGPU}, mucho más potentes, la nueva implementación no provoque consumos elevados de memoria y acelere el proceso.
...
El haber realizado la optimización sobre \textit{Google Colab} y no sobre los servidores \textit{nGPU} sobre los que finalmente correremos el aprendizaje y validación de modelos es un error a tener en cuenta. Por ejemplo, en \customref{isubs:segunda_ronda_optimizaciones} podemos ver que las conclusiones no son del todo robustas al trabajar en un entorno inferior y ser conocedores de que en futuro usaríamos un entorno mucho más potente.
